...
sub clipSet
{
    ...
    if ($^O eq "darwin") {
        # macOS
        open (PIPE, "|pbcopy") or die "Cannot open pbcopy: $!";
        print PIPE $text;
        close PIPE or die "Cannot close pbcopy: $!";
    }
    elsif ($^O eq "linux") {
        # Linux – requires xclip
        if (system("which xclip > /dev/null 2>&1") == 0) {
            open (PIPE, "|xclip -selection clipboard") or die "Cannot open xclip: $!";
            print PIPE $text;
            close PIPE or die "Cannot close xclip: $!";
            print "*** Copied to Linux clipboard. You can paste it now.\n";
        } else {
            print "*** xclip not installed. Showing text below:\n\n";
            print $text, "\n";
        }
    }
    else {
        # Other OS: just print to terminal
        print "*** Clipboard not supported. Showing text below:\n\n";
        print $text, "\n";
    }
